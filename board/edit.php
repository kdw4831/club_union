...
foreach($matches[1] AS $key=>$row){
    ...
    //$row='data:image/jpeg;base64,dkdsdsd ;
...
$imglist=implode('|', $img_array); // ['a','b','c']====> a|b|c  배열을 구분자 기준으로 해서 문자열로 만들어줌

if($pwd_hash !=''){
    $sql="UPDATE testboard SET name=:name, subject=:subject, content=:content,imglist=:imglist, password=:password WHERE idx=:idx";
}else{
    $sql="UPDATE testboard SET name=:name, subject=:subject, content=:content,imglist=:imglist WHERE idx=:idx";
